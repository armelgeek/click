...
export function HomePage() {
    return (
        <div className="min-h-screen  text-white px-4 py-4 flex flex-col gap-6">
            ...
            <Label icon={<Box className="text-vapo-purple-primary w-6 h-6" />} className="text-vapo-purple-primary text-md font-bold">
                Catégorie des produits populaires
            </Label>
            <div className="flex flex-row gap-6 justify-between mb-4">
                {[1, 2, 3, 4].map((_, i) => (
                    <div key={i} className="flex items-center justify-center">
                        <CategoryIcon label={`Category ${i + 1}`} />
                    </div>
                ))}
            </div>
            ...
            <div className="flex flex-row overflow-x-auto gap-3">
                ...
                <StoreCard name="Plaisance" image={`/icons/store.svg`}  />
