...
export default function CheckoutPage() {
  ...
  const [pendingContinueAfterAddressAdd, setPendingContinueAfterAddressAdd] = useState(false);
  ...
  const handleNextStep = async () => {
    if (currentStep === 'address') {
      // Si aucune adresse n'existe, forcer l'affichage du formulaire sans bloquer
      if (addresses.length === 0) {
        setShowAddressForm(true);
        setPendingContinueAfterAddressAdd(true);
        return;
      }
      // Si on est sur le formulaire d'ajout, valider ce formulaire
      if (showAddressForm && addressFormRef.current) {
        ...
        if (!isValid) {
          ...
          setPendingContinueAfterAddressAdd(false);
          return;
        }
        await addressFormRef.current.submit();
        // onSuccess callback va gérer la suite
        return;
      }
      // Sinon, vérifier qu'une adresse est sélectionnée
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      // Ensure we have an authenticated user for server validation and order
      if (!session?.user?.id) {
        setStockValidation(null);
        setPaymentError('Veuillez vous connecter pour valider le panier et effectuer la commande.');
      ...
      const stockResult = await CartAPI.validateStockThrottled(true, undefined, payload);
      setIsValidatingStock(false);
  ...
  // selectedAddress and selectedPayment are used in renderReviewStep for UI only
          ...
          <AddressForm
            ...
            onSuccess={async () => {
              ...
              if (result.data?.addresses && result.data.addresses.length > 0) {
                ...
                if (pendingContinueAfterAddressAdd) {
                  setPendingContinueAfterAddressAdd(false);
                  setTimeout(() => {
                    setCurrentStep('payment');
                  }, 100);
